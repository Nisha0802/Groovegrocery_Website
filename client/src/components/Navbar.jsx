        ...
        <nav className="fixed top-0 left-0 w-full z-50 flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white transition-all">
            ...
            <div className="hidden sm:flex items-center gap-8">
                ...
                <div onClick={() => navigate("/cart")} className="relative cursor-pointer">
                    ...
                    text-xs text-white bg-primary w-[18px] h-[18px] rounded-full">{getCartCount()}</button>
                </div>
                {/* wrrape this button we wrap with the help of { }*/}
            ...
            {open && (
                <div className={`${open ? 'flex' : 'hidden'} absolute top-[60px] left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden`}>
                    ...
                    {/* it means user is login !user - means user is not login*/}
                    {user &&
